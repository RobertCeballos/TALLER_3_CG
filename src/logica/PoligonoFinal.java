...
//             
//            
//            System.out.println("vectorIniciales: "+vIniciales.get(i));           
//        }       
    }
    ...
    void vPuntosFinales() {
        ...
        for (int i = 0; i < tamaño + 1; i++) {
            ...
            vPuntos.add(i,obtieneVectorFinal(vIniciales, idx));
            
             vPuntosFinales = (Vector) vPuntos.get(i);
                            ...
                            for(int r = 0; r < vPuntosFinales.size(); r++){
                                ...
                                EliminaFinales(vPuntos, idx);
                            }
             
            
        }
//        for (int i = 0; i < tamaño + 1; i++) {
//            Vector v = (Vector)vPuntos.get(i);
//            System.out.println("PuntosFinalBBBB en " + i + " es =" + v.size());
//        }

    }

    void EliminaFinales(Vector vec, int ind) {

        Vector eliminaV = new Vector();
        Vector eliminaV1 = new Vector();

        int tamañoV = vec.size();
        double[] arreglo = new double[10];

        eliminaV = (Vector) vec.get(tamañoV - 1);

        for (int i = 0; i < eliminaV.size(); i++) {
            arreglo = (double[]) eliminaV.get(i);

            if (k == 15) {
                k = 0;
            }
            System.out.println("indiceY: "+indiceY[k + 1]);
            if (arreglo[0] == indiceY[k + 1]) {

                eliminaV.remove(i);

                continue;
            }
            vPuntos.remove(vPuntos.size() - 1);
            vPuntos.add(eliminaV);
        }
        k++;
    ...
    Vector obtieneVectorFinal(Vector vectorF, int indi) {
        ...
        outt = (incrementoAutomatico(outAux, indi));
        return outt;
    }

    //Recibe el vector con los arreglos para ser calculados
    Vector incrementoAutomatico(Vector a, int b) {
